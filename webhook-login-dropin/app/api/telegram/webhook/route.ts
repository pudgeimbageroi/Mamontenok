...
interface MarkupRow {
  id: string;
  mode: "percent" | "fixed_rub" | "custom_rate";
...
async function handleRate(fromId: number) {
  ...
  const markup = mRes.data as MarkupRow | null;
  if (!rates || !markup) {
...
async function handleUpdate(fromId: number) {
  ...
  await sendRatesCard(fromId, newRates as RateRow, markup as MarkupRow, new Date(), true);
  return NextResponse.json({ ok: true });
}
