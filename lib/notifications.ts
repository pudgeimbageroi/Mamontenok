...
/**
 * Уведомление о событии со сделкой — с учётом её приватности.
 *
 * ⚠️ ВСЕ уведомления о сделках должны идти через эту функцию.
 * Ветвление живёт здесь, а не в вызывающем коде: иначе рано или поздно
 * кто-то забудет проверку, и личная сделка улетит в общую группу.
 *
 * Для личной сделки не отправляется НИЧЕГО — ни в группу, ни партнёру.
 * Владелец и так видит её в приложении.
 */
export async function notifyDealEvent(
  visibility: "joint" | "private" | null | undefined,
  actorTelegramId: number | undefined,
  text: string,
): Promise<void> {
  if (visibility === "private") {
    // Тишина. Осознанно, а не по недосмотру.
    return;
  }
  await notifyOtherPartners(actorTelegramId, text);
}
