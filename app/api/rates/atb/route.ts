...
export async function POST() {
  ...
  try {
    ...
    const { data: prev } = await supabase
      .from("rates")
      .select("atb_actual_rate, moex_cny_tod, moex_cny_tom, moex_cny_tms, moex_fetched_at, shage_rate")
    ...
    const { data, error } = await supabase
      ...
      .insert({
        ...
        // Не теряем биржевые курсы и курс 沙哥 при обновлении АТБ
        moex_cny_tod: prev?.moex_cny_tod ?? null,
        moex_cny_tom: prev?.moex_cny_tom ?? null,
        moex_cny_tms: prev?.moex_cny_tms ?? null,
        moex_fetched_at: prev?.moex_fetched_at ?? null,
